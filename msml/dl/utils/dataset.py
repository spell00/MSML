...
class MSDataset4(Dataset):
    def __init__(self, data, training=False, labels=None, batches=None, train_samples=None, concs=None, transform=None, quantize=False,
                 remove_paddings=False, crop_size=-1, add_noise=False, random_recs=False, device='cuda'):
        ...
        if train_samples is not None:
            labels_inds = {label: [i for i, (ts2, x) in enumerate(zip(train_samples, labels)) if x == label and ts2] for label in labels}
            self.labels_data = {label: data[labels_inds[label]].copy() for label in labels}
            self.n_labels = {label: len(self.labels_data[label]) for label in labels}
        else:
            self.labels_data = None
            self.n_labels = None
            self.train_samples = np.array([0 for _ in self.labels])
    ...
    def __getitem__(self, idx):
        ...
        if self.labels is not None:
            ...
            label = None
        if self.random_recs and self.train_samples[idx]:
            # print(self.pool.shape, len(self.pool))
            to_rec = self.labels_data[label][np.random.randint(0, self.n_labels[label])].copy()
            not_label = None
            while not_label == label or not_label is None:
            ...
            to_rec = self.samples[idx].copy()
            not_to_rec = np.zeros(to_rec.shape)
        x = self.samples[idx]
        if self.crop_size != -1:
            max_start_crop = x.shape[1] - self.crop_size
            ran = np.random.randint(0, max_start_crop)
        ...
        if self.add_noise:
            if np.random.random() > 0.5:
                x = x * (Variable(x.data.new(x.size()).normal_(0, 0.1)) > -.1).type_as(x)
        return x, label, batch, to_rec, not_to_rec, concs, self.train_samples[idx]
