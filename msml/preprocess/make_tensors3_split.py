...
def process_data(data, cats, model, cutoff, dirname, k=10000, binary=False, run_name='', combat_corr=0):
    ...
    features_scores.to_csv(
        f'{dirname}/{run_name}/{args.feature_selection}_scores_binary{str(binary)}.csv',
    ...
    features_scores_gt.to_csv(
        f'{dirname}/{run_name}/{args.feature_selection}_scores_binary{str(binary)}_gt{cutoff}.csv',
        index_label='minp_maxp_rt_mz')
...
def concat_and_transpose(dirinput, dirname, bins, args_dict, names_to_keep=None, features=None):
    """
...
if __name__ == "__main__":
    ...
    data_matrix = concat_and_transpose(dir_input, dir_name, bins=bins, args_dict=args,
                                       names_to_keep=bacteria_to_keep)
    if args.shift:
        data_matrix2 = concat_and_transpose(dir_input, dir_name, bins=bins, args_dict=args,
                                            names_to_keep=bacteria_to_keep)
        data_matrix3 = concat_and_transpose(dir_input, dir_name, bins=bins, args_dict=args,
                                            names_to_keep=bacteria_to_keep)
        data_matrix4 = concat_and_transpose(dir_input, dir_name, bins=bins, args_dict=args,
                                            names_to_keep=bacteria_to_keep)
    ...
    args.mutual_info_path = f'{dir_name}/{args.run_name}/{args.feature_selection}_scores_binary{str(args.binary)}.csv'
    if args.k > -1:
